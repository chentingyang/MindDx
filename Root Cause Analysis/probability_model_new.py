...
def feature_selection(data, kpi_list):
    ...
    samples = []
    labels = []
    for t in range(len(multidata.anomaly_type)):
        samples.extend(multidata.pattern_feature[t])
        labels.extend([t] * len(multidata.pattern_feature[t]))
...
lib_path = r'C:\Users\闫凌森\Desktop\diagnosis\DBPA_dataset-main\kpi_cluster_multidataset'
